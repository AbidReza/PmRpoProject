...
Background:
  ...
  Scenario:Verify import wizard document page
    ...
    And select "ATP" device option
    And user select sub device type dropdown
    And user select "ATP 2 - 64" sub device option
    And user checked exclude button
    And user checked exclude unit of usage button
    And user should input "20" minimum days supply
    And user should input "20" high cap drawer
    And user should input "20" smart cap drawer
    And user should input "20" days of use
    And user select day of use dropdown
    And user select "90" day of use option
